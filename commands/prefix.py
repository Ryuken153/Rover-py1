...
from db import set_prefixes, get_prefixes, add_prefixes, remove_prefixes
...
class Prefix(commands.Cog):
  ...
  async def setprefix(self, ctx: commands.Context, new prefix: str):
    await set_prefix(ctx.guild.id, new_prefix)
    await ctx.reply(f"Prefix set to `{new_prefix}`")

  @commands.command()
  @commands.has_permissons(manage_guild=True)
